...
4. 标注每个话题的核心贡献者（发言最多或提出关键观点的人），使用消息文件中的原始昵称
5. 为每个话题打 1-3 个标签（如：AI Agent、商业化、技术方案、行业趋势等）
6. 如果某些讨论没有实质知识价值（纯闲聊、表情等），直接跳过
...
def fetch_messages(report_date):
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        return list(conn.execute(
            "SELECT id, group_name, sender, sent_at, content, msg_type FROM messages "
            "WHERE group_name LIKE ? AND sent_at >= ? AND sent_at < ? ORDER BY sent_at ASC, id ASC",
            ("%智能体%", start, end),
        ))
...
def write_messages_file(messages, messages_path):
    ...
    for row in sampled:
        ...
        sender = (row["sender"] or "未知用户").strip() or "未知用户"
...
def generate(report_date):
    ...
    active_members = len({(row["sender"] or "").strip() for row in messages if (row["sender"] or "").strip()})
